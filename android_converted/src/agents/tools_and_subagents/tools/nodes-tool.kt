...
/**
 * Phase 2 Kotlin port for `src/agents/tools/nodes-tool.ts`.
...
// TODO(port-deps): type CameraFacing,
// TODO(port-deps): cameraTempPath,
// TODO(port-deps): parseCameraClipPayload,
// TODO(port-deps): parseCameraSnapPayload,
// TODO(port-deps): writeCameraClipPayloadToFile,
// TODO(port-deps): writeCameraPayloadToFile,
// TODO(port-deps): } from "../../cli/nodes-camera.js";
// TODO(port-deps): import { parseEnvPairs, parseTimeoutMs } from "../../cli/nodes-run.js";
// TODO(port-deps): import {
// TODO(port-deps): parseScreenRecordPayload,
// TODO(port-deps): screenRecordTempPath,
// TODO(port-deps): writeScreenRecordToFile,
// TODO(port-deps): } from "../../cli/nodes-screen.js";
// TODO(port-deps): import { parseDurationMs } from "../../cli/parse-duration.js";
// TODO(port-deps): import type { OpenClawConfig } from "../../config/config.js";
// TODO(port-deps): import { parsePreparedSystemRunPayload } from "../../infra/system-run-approval-context.js";
...
// TODO(port-deps): import { listNodes, resolveNode, resolveNodeId, resolveNodeIdFromList } from "./nodes-utils.js";
...
val NOTIFY_PRIORITIES = ["passive", "active", "timeSensitive"] as const;
val NOTIFY_DELIVERIES = ["system", "overlay", "auto"] as const;
val NOTIFICATIONS_ACTIONS = ["open", "dismiss", "reply"] as const;
val CAMERA_FACING = ["front", "back", "both"] as const;
val LOCATION_ACCURACY = ["coarse", "balanced", "precise"] as const;
...
suspend fun invokeNodeCommandPayload(params: {
  ...
  val raw = await callGatewayTool<{ payload: unknown }>("node.invoke", params.gatewayOpts, {
    nodeId,
...
fun extractPairingRequestId(message: string): string | null {
    ...
    execute: async (_toolCallId, args) => {
      ...
      try {
        switch (action) {
          ...
          case "notify": {
            val node = readStringParam(params, "node", { required: true });
            val title = typeof params.title == "string" ? params.title : "";
            val body = typeof params.body == "string" ? params.body : "";
          ...
          case "camera_snap": {
            ...
            val details: Array<Record<string, unknown>> = [];

            for (val facing of facings) {
              val raw = await callGatewayTool<{ payload: unknown }>("node.invoke", gatewayOpts, {
                nodeId,
                command: "camera.snap",
                params: {
                  facing,
            ...
            val result: AgentToolResult<unknown> = { content, details };
          ...
          case "run": {
            ...
            val needsScreenRecording =
              typeof params.needsScreenRecording == "boolean"
                ...
                : null;
            val prepareRaw = await callGatewayTool<{ payload?: unknown }>(
              "node.invoke",
              gatewayOpts,
              {
                nodeId,
        ...
        val gatewayLabel =
          gatewayOpts.gatewayUrl && gatewayOpts.gatewayUrl.trim()
            ...
            : "default";
        val agentLabel = agentId ?: "unknown";
