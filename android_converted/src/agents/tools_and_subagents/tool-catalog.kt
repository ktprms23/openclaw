...
/**
 * Phase 2 Kotlin port for `src/agents/tool-catalog.ts`.
 * Structural translation only; external dependencies remain as placeholders where needed.
 */

typealias ToolProfileId = Any /* TODO: translate TypeScript alias */

typealias ToolProfilePolicy = Any /* TODO: translate TypeScript alias */

typealias CoreToolSection = Any /* TODO: translate TypeScript alias */

typealias CoreToolDefinition = Any /* TODO: translate TypeScript alias */

val CORE_TOOL_SECTION_ORDER: Array<{ id: string; label: string }> = [
  { id: "fs", label: "Files" },
...
val CORE_TOOL_DEFINITIONS: CoreToolDefinition[] = [
  {
    id: "read",
    label: "read",
...
val CORE_TOOL_BY_ID = new Map<string, CoreToolDefinition>(
  CORE_TOOL_DEFINITIONS.map((tool) => [tool.id, tool]),
);

fun listCoreToolIdsForProfile(profile: ToolProfileId): string[] {
  return CORE_TOOL_DEFINITIONS.filter((tool) => tool.profiles.includes(profile)).map(
    (tool) => tool.id,
  );
}

val CORE_TOOL_PROFILES: Record<ToolProfileId, ToolProfilePolicy> = {
  minimal: {
...
fun buildCoreToolGroupMap() {
  val sectionToolMap = new Map<string, string[]>();
...
fun resolveCoreToolProfilePolicy(profile?: string): ToolProfilePolicy | null {
  if (!profile) {
    return null;
  }
  val resolved = CORE_TOOL_PROFILES[profile as ToolProfileId];
  if (!resolved) {
    return null;
  }
...
fun listCoreToolSections(): CoreToolSection[] {
...
fun resolveCoreToolProfiles(toolId: string): ToolProfileId[] {
