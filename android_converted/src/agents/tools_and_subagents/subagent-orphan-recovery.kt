...
/**
 * Phase 2 Kotlin port for `src/agents/subagent-orphan-recovery.ts`.
...
// TODO(port-deps): import { readSessionMessages } from "../gateway/session-utils.fs.js";
// TODO(port-deps): import { createSubsystemLogger } from "../logging/subsystem.js";
// TODO(port-deps): import { replaceSubagentRunAfterSteer } from "./subagent-registry.js";
...
fun extractMessageText(msg: unknown): string | null {
  if (!msg || typeof msg != "object") {
    return null;
  }
  val m = msg as Record<string, unknown>;
...
suspend fun recoverOrphanedSubagentSessions(params: {
  ...
  val resumedSessionKeys = params.resumedSessionKeys ?: new Set<string>();
...
fun scheduleOrphanRecovery(params: {
  ...
  val resumedSessionKeys = new Set<string>();

  val attemptRecovery = (attempt: number, delay: number) => {
    setTimeout(() => {
