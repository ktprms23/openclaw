...
/**
 * Phase 2 Kotlin port for `src/agents/tool-loop-detection.ts`.
...
// TODO(port-deps): import type { SessionState } from "../logging/diagnostic-session-state.js";
// TODO(port-deps): import { createSubsystemLogger } from "../logging/subsystem.js";
// TODO(port-deps): import { isPlainObject } from "../utils.js";
...
typealias ResolvedLoopDetectionConfig = Any /* TODO: translate TypeScript alias */

fun asPositiveInt(value: number | null, fallback: number): number {
 ...
 */
fun hashToolCall(toolName: string, params: unknown): string {
  return `${toolName}:${digestStable(params)}`;
}

fun stableStringify(value: unknown): string {
  ...
  val obj = value as Record<string, unknown>;
  val keys = Object.keys(obj).toSorted();
  return `{${keys.map((k) => `${JSON.stringify(k)}:${stableStringify(obj[k])}`).join(",")}}`;
}

fun digestStable(value: unknown): string {
  val serialized = stableStringifyFallback(value);
  return createHash("sha256").update(serialized).digest("hex");
}

fun stableStringifyFallback(value: unknown): string {
  try {
...
fun isKnownPollToolCall(toolName: string, params: unknown): boolean {
...
fun extractTextContent(result: unknown): string {
...
fun formatErrorForHash(error: unknown): string {
...
fun getToolCallStats(state: SessionState): {
  ...
  val history = state.toolCallHistory ?: [];
  val patterns = new Map<string, { toolName: string; count: number }>();
  ...
  var mostFrequent: { toolName: string; count: number } | null = null;
