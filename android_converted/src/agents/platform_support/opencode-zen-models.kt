...
// Source: src/agents/opencode-zen-models.ts
...
// TODO(openclaw-kotlin-port): import type { ModelApi, ModelDefinitionConfig } from "../config/types.js";
// TODO(openclaw-kotlin-port): import { createSubsystemLogger } from "../logging/subsystem.js";

val log = createSubsystemLogger("opencode-zen-models")

val OPENCODE_ZEN_API_BASE_URL = "https://opencode.ai/zen/v1"
val OPENCODE_ZEN_DEFAULT_MODEL = "claude-opus-4-6"
val OPENCODE_ZEN_DEFAULT_MODEL_REF = `opencode/${OPENCODE_ZEN_DEFAULT_MODEL}`

// Cache for fetched models (1 hour TTL)
var cachedModels: ModelDefinitionConfig[] | Nothing? = Nothing?
var cacheTimestamp = 0
val CACHE_TTL_MS = 60 * 60 * 1000 // 1 hour

/**
 * Model aliases for convenient shortcuts.
 * Users can use "opus" instead of "claude-opus-4-6", etc.
 */
val OPENCODE_ZEN_MODEL_ALIASES: Map<String, String> = {
  // Claude
 ...
 */
fun resolveOpencodeZenAlias(modelIdOrAlias: String): String {
  val normalized = modelIdOrAlias.toLowerCase().trim()
 ...
 */
fun resolveOpencodeZenModelApi(modelId: String): ModelApi {
...
val DEFAULT_COST = { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 }

val MODEL_CONTEXT_WINDOWS: Map<String, Double> = {
...
fun getDefaultContextWindow(modelId: String): Double {
  return MODEL_CONTEXT_WINDOWS[modelId] ?? 128000
}

val MODEL_MAX_TOKENS: Map<String, Double> = {
...
fun getDefaultMaxTokens(modelId: String): Double {
 ...
 */
fun buildModelDefinition(modelId: String): ModelDefinitionConfig {
  return {
    id: modelId,
 ...
 */
val MODEL_NAMES: Map<String, String> = {
 ...
 */
fun getOpencodeZenStaticFallbackModels(): ModelDefinitionConfig[] {
  val modelIds = [
    "gpt-5.1-codex",
    "claude-opus-4-6",
    "claude-opus-4-5",
 ...
 */
suspend fun fetchOpencodeZenModels(apiKey?: String): Promise<ModelDefinitionConfig[]> {
  ...
  try {
    ...
    val response = await fetch(`${OPENCODE_ZEN_API_BASE_URL}/models`, {
      method: "GET",
      headers,
    ...
    val data = (await response.json()) as ZenModelsResponse

    if (!data.data || !Array.isArray(data.data)) {
      throw error("Invalid response format from /models endpoint")
    }

    val models = data.data.map((model) => buildModelDefinition(model.id))
 ...
 */
fun clearOpencodeZenModelCache(): Unit {
  cachedModels = Nothing?
  cacheTimestamp = 0
}
